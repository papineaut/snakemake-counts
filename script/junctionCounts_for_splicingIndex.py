...
for line in iIntrons:
    ...
    gene = line.split('\t')[3].split('_')[0] # for GENCODE
    intron = line.split('\t')[3].split('_')[2]  # intron number within the gene
        ...
        if pos5SS-read.pos<=100 or read.aend-pos5SS<=100: 
            ...
            if pos5SS-read.pos>=3 and read.aend-pos5SS>=3:
                ...
                # if read is in the sense direction for the intron    
                    ...
                    if (strand=='+' and read.is_reverse and read.is_read1
                        or (strand=='+' and not read.is_reverse and read.is_read2)
                        or (strand=='-' and read.is_reverse and read.is_read2)
                        or (strand=='-' and not read.is_reverse and read.is_read1)):
    ...
    oFile.write(str(gene)+'\t'+str(int(intron)+1)+'\t'+str(chrom)+'\t'+str(start)+'\t'+str(end)+'\t'+str(strand)+'\t'+str(unspliced5SS)+'\t'+str(unspliced3SS)+'\t'+str(splice5SS)+'\n')
